...
function openModal(hotel, priceText, isExternal = false) {
      ...
      <div class="modal-grid">
        <div class="modal-section">
          ...
          <ul class="detail-list">
            ...
            <li class="price-highlight" id="modal-price-container" style="display:flex; justify-content:space-between; align-items:flex-start;">
              ...
              <div id="modal-price-value" style="display: flex; flex-direction: column; align-items: flex-end; text-align: right;">
                 <span style="color: #10b981; font-weight: 700; font-size: 1.05rem; display:flex; align-items:center; gap:0.4rem;">
  ...
  // Fetch real price and update the price reference section
  setTimeout(() => {
    fetch('/api/realprice?name=' + encodeURIComponent(hotel.Name))
      .then(r => r.json())
      .then(data => {
        const container = document.getElementById('modal-price-value');
        if (!container) return;
        
        if (data && data.minPrice) {
          const simBadge = data.isSimulated ? '<span style="font-size:0.75rem; background:rgba(255,255,255,0.2); color:#cbd5e1; padding:0.1rem 0.3rem; border-radius:4px; margin-left:0.4rem; font-weight:normal;">預估價格</span>' : '';
          let roomTypeHtml = data.roomType ? `<div style="font-size:0.85rem; color:#6ee7b7; font-weight:normal; margin-bottom:0.2rem;">房型：${data.roomType}</div>` : '';
          
          let linksHtml = `<div style="display:flex; gap:0.8rem; margin-top:0.8rem; flex-wrap:wrap; background: rgba(255,255,255,0.03); padding: 0.8rem 1rem; border-radius: 8px; border: 1px solid var(--border-light); justify-content: flex-start; align-items: center;">`;
          linksHtml += `<span style="color: var(--text-muted); font-size: 0.85rem; margin-right: auto;">前往訂房：</span>`;
          linksHtml += `<a href="https://www.booking.com/searchresults.zh-tw.html?ss=${encodeURIComponent(hotel.Name)}" target="_blank" style="color:#60a5fa; text-decoration: underline; font-size: 0.9rem; font-weight: 500;">Booking.com</a>`;
          linksHtml += `<a href="https://www.klook.com/zh-TW/search/result/?query=${encodeURIComponent(hotel.Name)}" target="_blank" style="color:#fb923c; text-decoration: underline; font-size: 0.9rem; font-weight: 500;">Klook</a>`;
          linksHtml += `<a href="https://www.google.com/search?q=${encodeURIComponent(hotel.Name + ' 住宿 價格')}" target="_blank" style="color:#34d399; text-decoration: underline; font-size: 0.9rem; font-weight: 500;">Google 搜尋</a>`;
          linksHtml += `</div>`;

          container.innerHTML = `
            ${roomTypeHtml}
            <div style="color: #10b981; font-weight: 700; font-size: 1.25rem;">NT$ ${data.minPrice.toLocaleString()} <span style="font-size:0.85rem; font-weight:normal;">/ 1人1晚</span> ${simBadge}</div>
          `;
          
          const linksContainer = document.getElementById('modal-price-links');
          if (linksContainer) {
            linksContainer.innerHTML = linksHtml;
          }
        } else {
          container.innerHTML = `<span style="color: #ef4444; font-size: 1rem; font-weight:bold;">${priceText}</span>`;
        }
      })
      .catch(() => {
        const container = document.getElementById('modal-price-value');
        if (container) container.innerHTML = `<span style="color: #10b981; font-weight: 700; font-size: 1.15rem;">${priceText}</span>`;
      });
  }, 100);
