...
app.get('/api/realprice', async (req, res) => {
    ...
    try {
        const { data } = await axios.get(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(hotelName + ' agoda booking 價格 NT$')}`, {
            headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36' },
            timeout: 5000
        });
        const $ = cheerio.load(data);
        ...
        $('.result__snippet, .result__title').each((i, el) => {
            const text = $(el).text();
            const match = text.match(/(?:NT\$|TWD|\$|台幣|價格)\s*([0-9,]{3,})/i);
            if (match) {
                const p = parseInt(match[1].replace(/,/g, ''));
                if (p >= 500 && p <= 30000 && p < minPrice) {
                    minPrice = p;
                    found = true;
        ...
        if (found) {
            console.log(`[後端執行狀況] 成功爬取到即時價格: ${platform} - NT$${minPrice}`);
            return res.json({ price: minPrice, platform });
        } else {
            console.log(`[後端執行狀況] 查無明顯即時價格，套用系統預估模型`);
            // 若爬蟲被擋或無結果，使用與附近相同的穩定雜湊模擬價格，保持一致性
            let hash = 0;
            for (let i = 0; i < hotelName.length; i++) {
                hash = ((hash << 5) - hash) + hotelName.charCodeAt(i);
                hash |= 0;
            }
            const simPrice = 1000 + (Math.abs(hash) % 30) * 100;
            const platforms = ['Agoda', 'Booking.com', 'Klook', 'Trip.com'];
            const simPlatform = platforms[Math.abs(hash) % platforms.length];
            return res.json({ price: simPrice, platform: simPlatform, isSimulated: true });
        }
    } catch (err) {
